...
5. **Тільки в цьому демо:** показує кнопку "Обміняти code на токени" → юзер натискає → браузер надсилає `POST /exchange`.

**Що бекенд ще НЕ зробив?** Не звертався до Keycloak. `code` лише отримано і показано.

> **Різниця демо vs реальний BFF:**
> У реальному BFF (або Spring Security `oauth2Login()`) кроки 3 і 4 відбуваються **в одному `/callback` handler'і** — BFF автоматично викликає Keycloak, зберігає токени, і відразу редіректить браузер на оригінальний URL. Ніякого `POST /exchange` не існує, юзер code взагалі не бачить. В цьому демо обмін навмисно виділено в окремий крок, щоб показати що саме відбувається всередині.
...
> ⚠️ **Цей крок специфічний для демо.** `POST /exchange` ініціює браузер (кнопка на `callback.html`), але тільки тому що демо штучно зупиняється щоб показати code. У реальному BFF BFF сам ініціює обмін всередині `/callback` — без окремого браузерного запиту.
...
6. Повертає `exchange-result.html` — HTTP-відповідь на **той самий `POST /exchange` запит браузера**.
7. Браузер бачить лише красиво відформатований JSON на сторінці — **не сам токен у cookie чи в JS**.

**Що таке back-channel і чому це REST?**
Back-channel — це звичайний HTTP-запит (`RestClient.post()`), але не від браузера, а від Java-коду BFF до Keycloak. Браузер цього запиту взагалі не бачить (не в Network DevTools, нікуди). Весь крок 4 відбувається **всередині одного browser request-response циклу** — браузер надіслав `POST /exchange` і «чекає» відповіді. Поки браузер чекає, BFF-потік синхронно викликає Keycloak, отримує токени, зберігає в сесію, і лише тоді повертає HTTP 200 з `exchange-result.html` браузеру. Тобто:

```
Браузер              BFF                      Keycloak
  |                   |                          |
  |-- POST /exchange->|                          |
  |   (code у тілі)   |-- POST /token (REST) --->|
  |                   |   (code + client_secret) |
  |     чекає...      |<-- 200 JSON (tokens) ----|
  |                   | [зберігає в HttpSession]  |
  |<-- 200 OK --------|                          |
  |  (exchange-result.html)
```
