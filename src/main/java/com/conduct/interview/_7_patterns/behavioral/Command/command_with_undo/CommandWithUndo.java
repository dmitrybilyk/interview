...
public class CommandWithUndo {
  public static void main(String[] args) {
    ...
    TextEditorCommand textEditorCommand =
        new TextEditorInsertCommand(
            textEditor,
            "Initial Text",
            textEditor.getContent().isEmpty() ? 0 : textEditor.getContent().length());
    commandInvoker.insertText(textEditorCommand);

    // Insert additional text
    TextEditorCommand textEditorCommand2 =
        new TextEditorInsertCommand(
            textEditor,
            "Additional Text",
            textEditor.getContent().isEmpty() ? 0 : textEditor.getContent().length());
    ...
    TextEditorCommand textEditorCommand3 =
        new TextEditorInsertCommand(
            textEditor,
            "One More Text",
            textEditor.getContent().isEmpty() ? 0 : textEditor.getContent().length());
...
class TextEditor {
  private final StringBuilder textContent =
      new StringBuilder(); // Using StringBuilder for efficient string manipulation
