...
public class GroupBy {
  public static void main(String[] args) {
    ...
    //        List<String> partitioned = givenList.stream().collect(partitioningBy(s -> s.length() >
    // 1))
    //                .get(true);
    //
    ...
    //        System.out.println(employeeList.stream().collect(Collectors.groupingBy(employee ->
    // employee.getSalary())));
    ...
    Map<Integer, Integer> integerIntegerMap =
        employeeList.stream()
            .collect(
                teeing(
                    counting(),
                    groupingBy(employee -> employee.getSalary()),
                    (aLong, aLong2) -> {
    ...
    List<Employee> distinctElements =
        employeeList.stream()
            .filter(distinctByKey(cust -> cust.getSalary()))
