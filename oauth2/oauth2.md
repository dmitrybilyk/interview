...
### 📌 Що таке OAuth 2.0?
**OAuth 2.0** — це протокол **авторизації**, який дозволяє додатку (Client) отримати обмежений доступ до захищених 
ресурсів користувача без передачі свого пароля (користувача). Пароль перевіряє окремий **Authorization Server** 
(Google, Keycloak) — він лише логінить юзера і видає токен. Самі ж дані віддає **Resource Server** — 
це може бути сервіс того ж провайдера (Google Calendar API належить тому ж Google), а може бути й повністю окрема система 
(напр. Keycloak лише видає токен, а перевіряє його вже ваш власний Java/Kotlin backend).
*   **Ключова ідея:** Делегування прав (Delegation of Permissions).
*   **OIDC (OpenID Connect):** Надбудова над OAuth 2.0, яка додає шар **автентифікації** (ID Token), 
щоб знати, *хто* цей користувач.
